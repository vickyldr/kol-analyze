...
OUTPUT_SCHEMA_HINT = {
    ...
    "staffing_section": {
        "overview": "五、人力分工与调整建议的总述：当前分工是否匹配各语言现状；机会语言(覆盖缺口/高潜)该分给谁",
        "people": [
            {"person": "负责人姓名",
             "suggestion": "给这个人【排好优先级、别贪多】的分工建议。硬约束：一个人最多同时扛约 3 个国家/语言，"
                           "且重要国家（消耗大/主力/加量）很吃精力——建议『2 个重要』或『1 重要 + 2 次要』的组合；"
                           "需要快速测试的临时国家只算『轻量兼顾』。明确写出：主力做哪 1-2 个（重点）、"
                           "兼顾/轻测哪些、这期先放掉哪些，而不是把所有建议都堆给他。"}
        ],
    },
    "langs": [
        {
...
def extra_block(extra: str) -> str:
    """本期补充说明 / 额外背景与要求（务必纳入分析与建议）。"""
    extra = (extra or "").strip()
    if not extra:
        return ""
    return ("【本期补充说明 · 额外背景与要求（务必纳入分析、缺口、人力与 todo）】\n"
            "这是我本期额外交代的背景/要求（如：领导要求新做/必须做的国家、要砍的国家、"
            "广告侧硬要做的方向、某脚本命名纠正等）。请把它当成硬约束纳入判断：\n"
            + extra + "\n")
